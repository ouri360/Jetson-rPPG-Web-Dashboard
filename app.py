...
detector = FaceDetector(decimation_rate=3)
processor = SignalProcessor(buffer_seconds=20, target_fps=30.0)
...
def generate_video_feed():
    ...
    while True:
        ...
        if not success or frame is None:
            time.sleep(
                0.01
            )  # Petite pause pour soulager le CPU si la caméra est en retard
            continue
        ...
        multi_rois = detector.get_face_mesh_rois(
            frame, draw=True, ai_weights=current_weights
        )
                        ...
                        if signals is not None:
                            ...
                            if len(sig_math) > 30:
                                ...
                                if bpm is not None:
                                    ...
                                    telemetry_data["snr"] = round(
                                        np.max(filt_mag) / (np.mean(filt_mag) + 1e-8), 2
                                    )
                                    ...
                                    # On convertit les fréquences en BPM pour l'affichage
                                    freqs_bpm = (freqs * 60).tolist()

                                    # On normalise la magnitude pour un affichage propre (0 à 1)
                                    mag_norm = (
                                        filt_mag / (np.max(filt_mag) + 1e-8)
                                    ).tolist()

                                    fft_labels = [
                                        [f"{round(f * 60, 1)} BPM", f"{round(f, 2)} Hz"]
                                        for f in freqs
                                    ]

                                    telemetry_data["fft_labels"] = fft_labels
                                    telemetry_data["fft_mag"] = [
                                        round(m, 3) for m in mag_norm
                                    ]
                                    # -------------------------------------------------

                                    telemetry_data["weights"] = (
                                        processor.get_latest_weights()
                                    )
